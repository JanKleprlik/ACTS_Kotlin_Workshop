...
you have (83, 86, 6, 31, 17, 9, 48, and 53). Of the numbers you have, four of them (48, 83, 17, and 86) are winning numbers!
...
fun demo_02(input: String): Int {
    ...
    for (lineIdx in lines.indices) {
        ...
        val winning: MutableSet<Int> = mutableSetOf()
        val myNumbers: MutableList<Int> = mutableListOf()
        var isFirstPart = true;
        for (tokenIdx in 2..<tokens.size) {
            if (tokens[tokenIdx] == "") {
                continue
            }
            if (tokens[tokenIdx] == "|") {
            ...
            if (isFirstPart)
                winning.add(tokens[tokenIdx].toInt())
            else
                myNumbers.add(tokens[tokenIdx].toInt())
        }

        var matches = 0
        for (i in 0..<myNumbers.size) {
            if (winning.contains(myNumbers[i])) {
                matches++
            }
        }
        var currentPoints = 0
